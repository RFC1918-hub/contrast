...
### Summary: 
E.Tree was a Python Flask application that used XPATH to parse XML files. We where presented with a example XML file from where we could see that some users have a additional selfDestructCode element set. Knowing this we where able to do a error-based XPATH injection to determine the flag.  

#### What is XPATH: 
The data stored in XML can be queried via XPath which is similar to SQL conceptually. _It is also a query language and is used to locate specific elements in a XML document._ There are no access level permissions and it is possible to refer almost any part of an XML document unlike SQL which allows restrictions on databases, tables or columns.
...
![Screenshot](../images/Pasted image 20210421112816.png)

Sending the request to BURP we can see the data is being send through as JSON. 

![Screenshot](../images/Pasted image 20210421120714.png)

Doing some fuzzing on special characters we can see that a quotation mark will break the application. 

![Screenshot](../images/Pasted image 20210421120610.png)
...
I send the following payload to the application. 
...
![Screenshot](../images/Pasted image 20210421121348.png)
...
We first test if we are able to get a success message or a query we know in true. We search for the user "Straorg" from the Leaderboard tab.
...
![Screenshot](../images/Pasted image 20210421121812.png)

Now searching for a none existing user. 
...
![Screenshot](../images/Pasted image 20210421121907.png)

Success. We now know if we send it a query thats true we will get a success and if the query is false we get a failure. Taking this knowledge we can now build a script to bruteforce each character of the flag. 
...
![Screenshot](../images/Pasted image 20210421122300.png)
...
![Screenshot](../images/Pasted image 20210421122458.png)
